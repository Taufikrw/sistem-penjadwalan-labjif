                    ...
                    <tbody>
                        @foreach ($schedules as $schedule)
                            <tr>
                                ...
                                <td class="border px-4 py-2">{{ $schedule->day }} {{ $schedule->start_time->format('H:i A') }} - {{ $schedule->end_time->format('H:i A') }}</td>
                                <td class="border px-4 py-2">
                                    <a href="{{ route('schedule.edit', $schedule->id) }}"
                                        class="text-blue-500 hover:underline">Edit</a>
                                    <form action="{{ route('schedule.delete', $schedule->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
